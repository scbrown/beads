...
@pytest.mark.asyncio
async def test_cli_works_in_worktree(git_worktree_with_separate_dbs, bd_executable):
    """Test that bd CLI works correctly in worktrees."""
    main_repo, worktree, temp_dir = git_worktree_with_separate_dbs

    # Create issue via CLI
    result = subprocess.run(
        ["bd", "create", "Worktree CLI issue", "-p", "1", "--json"],
        cwd=worktree,
        capture_output=True,
        text=True,
    )
    
    assert result.returncode == 0, f"CLI create should work: {result.stderr}"
    ...
    assert result.returncode == 0, f"CLI list should work: {result.stderr}"
