...
// InProgressIssues, StatusBlockedIssues, ClosedIssues, DeferredIssues,
// HookedIssues, PinnedIssues) of stats from
...
func ScanIssueCountsInTx(ctx context.Context, tx DBTX, stats *types.Statistics) error {
	if err := tx.QueryRowContext(ctx, `
		SELECT
			...
			COALESCE(SUM(CASE WHEN status = 'hooked' THEN 1 ELSE 0 END), 0),
